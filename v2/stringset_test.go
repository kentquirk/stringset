...
func TestOperations(t *testing.T) {
	...
	inter2 := ss2.Intersection(ss1)
	assert.True(t, inter2.Equals(New().Add("this", "test")))
	union := ss1.Union(ss2)
...
func TestJoin(t *testing.T) {
	ss1 := New().Add("a", "join", "test")
	s := ss1.Join(" ")
	assert.Regexp(t, "[a-z]+ [a-z]+ [a-z]+", s)
}

func TestWrappedJoin(t *testing.T) {
	ss1 := New().Add("a", "wrapped", "join", "test")
	s := ss1.WrappedJoin("LEFT", " ", "RIGHT")
	assert.Regexp(t, "LEFT[a-z]+ [a-z]+ [a-z]+ [a-z]+RIGHT", s)
}
